...
pagetitle: DDD CFPLs
subtitle: |
  [Pdf version of this document](https://github.com/ChandlerLutz/differences-in-differences-in-differences-CFPLs/blob/master/CFPLs_DDD.pdf)
  
  [Github repository for this document](https://github.com/ChandlerLutz/differences-in-differences-in-differences-CFPLs)
...
This case study will use a
difference-in-differences-in-differences(DDD) research design to study
the impact of the 2008 California Foreclosure Prevention Laws (CFPLs)
that aimed to reduce foreclosures at the height of the financial
crises. These laws are called the California Foreclosure Prevention
Laws. We first start with a classic difference-in-differences analysis
and then cover the DDD approach.
...
* [The Github Repository for this project is here](https://github.com/ChandlerLutz/differences-in-differences-in-differences-CFPLs).
*
  [Download the most recent pdf version here](https://github.com/ChandlerLutz/differences-in-differences-in-differences-CFPLs/blob/master/CFPLs_DDD.pdf).
*
  [Data can be downloaded in either rds or csv here](https://github.com/ChandlerLutz/differences-in-differences-in-differences-CFPLs/tree/master/data-raw).
  
This tutorial will employ the `R` statistical computing environment and
the `data.table` and `magrittr` packages for regression as well as the
`AER` package for regression analysis. 
...
foreclosure. Our aim is to analyze the effects of these policies on
foreclosures
(See
...
#Read in the Data
DT <- fread("https://raw.githubusercontent.com/ChandlerLutz/differences-in-differences-in-differences-CFPLs/master/data-raw/county_forc_cfpl_data.csv")
```
...
* `CFPL` -- and indicator that equals `1` for the CFPL treatment
  period 
* `zillow.forc` -- the real estate owned (RE0) foreclosures per 10,000
  homes 
...
`zillow.forc`. We will use counties in Arizona and Nevada as the
control group and counties in California as the treatment group. 
...
$\hat{\delta}$ is the differences-in-differences estimator (the
...
(2.73 times more, how did I get that number?) foreclosures than low
foreclosure counties. This makes sense, but suggests that our parallel
pre-trends assumption is violated. 
...
estimators; hence the name "Differences-in-differences-in-differences"
...
1. We can assess the parallel pre-trends assumption: During the
   ...
   high foreclosures is `(Intercept) + forc.high`; while the
   pre-treatment mean of foreclosures for high foreclosure California
   counties is `(Intercept) + forc.high + CA + forc.high:CA`. The
   difference, and what we need to test for the parallel pre-trends, is
   $H_0$: `CA + forc.high:CA` $= 0$. The F-statistics for null that
...
2. The DDD estimate is `-756.3945` and statistically significant at
...
3. Generally, in both DD and DDD research designs, you add extra
